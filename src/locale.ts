...
export default class Locale {
    ...
    noAccountText = (lang: String = 'es') => {
        return lang === 'es' ? 'Aún no tienes cuenta?' : 'Don\'t have an account yet?';
    }
    ...
    logoutPromptMessage = (lang: String = 'es') => {
        return lang === 'es' ? 'Está seguro de cerrar sesión?' : 'Are you sure to logout?';
    }
    ...
    emptyNameMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar su nombre' : 'You must enter your name';
    }
    emptyEmailMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar su correo electrónico' : 'You must enter your email';
    }
    emptyPasswordMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar su contraseña' : 'You must enter your password';
    }
    noLengthPasswordMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'La contraseña debe tener mínimo 8 caracteres' : 'Password must be at least 8 characters';
    }
    ...
    notMatchingPasswordsMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Las contraseñas deben coincidir' : 'Passwords must match';
    }
    emailAlreadyRegisteredMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'El correo electrónico ya está registrado' : 'Email already registered';
    }
    ...
    noLengthValidationCode = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar los 8 dígitos del código de validación' : 'You must enter the 8 digits of the validation code';
    }
    noLengthRecoveryCode = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar los 8 dígitos del código de recuperación' : 'You must enter the 8 digits of the recovery code';
    }
    ...
    recoveryPasswordOnCodeMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Le hemos enviado un código de recuperación al correo electrónico suministrado' : 'We have sent a recovery code to the email provided';
    }
    recoveryPasswordOnPasswordlMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Ingrese su nueva contraseña' : 'Enter your new password';
    }
    emailNotRegisteredMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Correo electrónico no registrado' : 'Email not registered';
    }
    passwordUpdatedMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Su contraseña ha sido actualizada exitosamente' : 'Your password has been successfully updated';
    }
    registerValidatedMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Su registro ha sido validado exitosamente. Ahora puede iniciar sesión' : 'Your registration has been successfully validated. You can now log in';
    }
    ...
    vehicleFuelConsumptionLabel = (lang: String = 'es') => {
        return lang === 'es' ? 'Consumo de Combustible (Km por 1 Litro)' : 'Fuel Consumption (Km per 1 Liter)';
    }
    vehicleFuelCostLabel = (lang: String = 'es') => {
        return lang === 'es' ? 'Costo de Combustible (1 Litro)' : 'Fuel Cost (1 Liter)';
    }
    ...
    noDeviceModelSelectedMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe seleccionar un modelo de dispositivo' : 'Must select a device model';
    }
    emptyDeviceImeiFieldMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar el imei del dispositivo' : 'Must enter the device imei';
    }
    emptySimcardGsmNumberFieldMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar el número gsm de la tarjeta sim' : 'Must enter the simcard gsm number';
    }
    emptySimcardCarrierFieldMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar la operadora de la tarjeta sim' : 'Must enter the simcard carrier';
    }
    emptySimcardApnFieldMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar el apn de la tarjeta sim' : 'Must enter the simcard apn';
    }
    emptyVehicleDescriptionFieldMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar la descripción del vehículo' : 'Must enter the vehicle description';
    }
    emptyVehicleLicensePlateFieldMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar la matrícula del vehículo' : 'Must enter the vehicle license plate';
    }
    deviceSavedSuccessfullyMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Dispositivo guardado exitosamente' : 'Device successfully saved';
    }
    deviceUpdatedSuccessfullyMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Dispositivo actualizado exitosamente' : 'Device successfully updated';
    }
    groupSavedSuccessfullyMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Grupo guardado exitosamente' : 'Group successfully saved';
    }
    groupUpdatedSuccessfullyMsg = (lang: String = 'es') => {
        return lang === 'es' ? 'Grupo actualizado exitosamente' : 'Group successfully updated';
    }
    ...
    dateFromGreaterDateToMessage = (lang: String = 'es') => {
        return lang === 'es' ? 'Fecha y Hora \'Desde\' debe ser menor que la Fecha y Hora \'Hasta\'' : 'Date and Time \'From\' must be smaller than Date and Time \'To\'';
    }
    ...
    deviceHistoryErrorMessage = (lang: String = 'es') => {
        return lang === 'es' ? 'Ocurrió un error al cargar el historial del dispositivo' : 'An error occurred while getting device history';
    }
    noDeviceHistoryTitle = (lang: String = 'es') => {
        return lang === 'es' ? 'Sin historial' : 'No history';
    }
    noDeviceHistoryMessage = (lang: String = 'es') => {
        return lang === 'es' ? 'No se encontraron resultados' : 'No results were found';
    }
    ...
    noDevicesToShowMessage = (lang: String = 'es') => {
        return lang === 'es' ? 'No hay dispositivos para mostrar' : 'No devices to show';
    }
    ...
    groupNameEmptyMessage = (lang: String = 'es') => {
        return lang === 'es' ? 'Debe ingresar el nombre del grupo' : 'Must enter the group name';
    }
    groupDeletePromptTitle = (lang: String = 'es') => {
        return lang === 'es' ? 'Eliminar Grupo' : 'Delete Group';
    }
    groupDeletePromptQuestion = (lang: String = 'es') => {
        return lang === 'es' ? 'Está seguro de eliminar el grupo' : 'Are you sure to delete the group';
    }
    groupDuplicatedMessage = (lang: String = 'es') => {
        return lang === 'es' ? 'Ya existe un grupo con el mismo nombre' : 'There is already a group with the same name';
    }
